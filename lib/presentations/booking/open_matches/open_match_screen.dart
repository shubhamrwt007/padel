...
class _OpenMatchesScreenState extends State<OpenMatchesScreen> {
  ...
  Widget _buildMatchCardFromData(BuildContext context, MatchData data, int index) {
    ...
    final timeStr = '${data.bookingId?.startTime??""}-${data.bookingId?.endTime??""}';
      ...
      child: Stack(
        children: [
          Align(
              ...
              child: SvgPicture.asset(Assets.images.imgOpenMatchBg.path,height:_isLoginUserInMatch(data)?160: 150,width: 150,).paddingOnly(right: 20)),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
        ...
        children: [
          ...
          Row(
            children: [
              ...
              SizedBox(width: 8),
              Text("${(data.teamA?.length ?? 0) + (data.teamB?.length ?? 0)} attendee", style: Get.textTheme.bodySmall!.copyWith(fontWeight: FontWeight.w400,fontSize: 11)),
            ],
          ),
          const SizedBox(height: 8),
          Container(
